...
static void tamagotchi_p1_hal_log(log_level_t level, char* buff, ...) {
    ...
    va_list args;
    va_start(args, buff);
    furi_string_vprintf(string, buff, args);
    va_end(args);
...
static void tamagotchi_p1_hal_play_frequency(bool_t en) {
    // Stay silent while catching up on elapsed time
    bool audible = en && g_ctx->ff_ticks == 0;

    if(audible && g_ctx->volume > 0) {
        if(furi_hal_speaker_is_mine() || furi_hal_speaker_acquire(30)) {
            float vol = (g_ctx->volume == 2) ? 1.0f : 0.4f;
            furi_hal_speaker_start(g_ctx->frequency, vol);
        }
    } else {
